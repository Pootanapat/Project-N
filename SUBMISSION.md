# รายละเอียดการส่งงาน

โปรเจกต์: Project-N — Brain Booster / Little Brainiac

สมาชิก:

- นาย เจ้านาย เอี่ยมสำอางค์ (68030049)
- นาย ภูธนพัตน์ ตากิ่มนอก (68030231)
- นาย ธนภัทร ดิสโร (68030116)

Release: v1.0.0

ไฟล์ที่แนบสำหรับการส่งงาน:
- `docs/brain-web/` (production build)
- `docs/little-web/` (production build)
- `README.md` (คู่มือสั้น ๆ)

วิธีตรวจสอบ:
1. เปิดลิงก์ GitHub Pages:
   - https://pootanapat.github.io/Project-N/brain-web/
   - https://pootanapat.github.io/Project-N/little-web/
2. หรือดาวน์โหลดไฟล์ release (zip) แล้วรันบนเครื่อง

หมายเหตุ:
- ระบบ CI จะ build และ deploy อัตโนมัติเมื่อมีการ push ไปยัง `main` (ใช้ GitHub Actions)
- หากต้องการให้ผมสร้าง Release และอัปโหลดไฟล์ zip ไปยังหน้า Releases ผมสามารถเตรียมไฟล์ให้และอัปโหลดให้ได้ (การอัปโหลดผ่าน API อาจต้องใช้ Personal Access Token)